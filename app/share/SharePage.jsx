...
export default function SharePage({ searchParams }) {
  ...
  // Para display visual: URL directa de iTunes o artwork pasado por Flutter
  // Para meta OG de WhatsApp: /api/og (con texto) — se usa en page.jsx
  const artworkUrl = artwork
    ? artwork   // URL directa de iTunes pasada por Flutter
    : `https://itunes.apple.com/search?term=${encodeURIComponent(`${artist} ${title}`)}&entity=song&limit=1&media=music`;

  // URL de la portada para mostrar en pantalla
  const [displayArtwork, setDisplayArtwork] = useState('');

  // Buscar portada en iTunes si no viene en params
  useEffect(() => {
    if (artwork) {
    ...
    fetch(`https://itunes.apple.com/search?term=${q}&entity=song&limit=3&media=music`)
      ...
      .then(data => {
        ...
        for (const r of results) {
          ...
          if (img) {
            setDisplayArtwork(img.replace('100x100bb', '600x600bb'));
            return;
          }
        }
      })
        ...
        <div className="artwork-section">
          <div className="artwork-wrap">
            <img
              ...
              style={{ opacity: (imgLoaded && displayArtwork) ? 1 : 0 }}
            />
{(!imgLoaded || !displayArtwork) && (
